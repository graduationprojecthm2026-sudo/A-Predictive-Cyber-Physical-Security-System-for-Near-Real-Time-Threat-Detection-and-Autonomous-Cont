...
PLAYBOOKS: Dict[str, dict] = {
    ...
    "data_exfil_response": {
        ...
        "triggers":    ["insider_threat", "coordinated_attack"],
        "min_severity":"HIGH",
        "steps": [
            {"step": 1, "action": "block_exfil_destination",
             "description": "Block external IP receiving stolen data",
             "auto": True},
            {"step": 2, "action": "isolate_source_host",
             "description": "Network-isolate the endpoint sending data",
             "auto": True},
            {"step": 3, "action": "capture_network_evidence",
             "description": "Full packet capture on affected segment",
             "auto": True},
            {"step": 4, "action": "notify_data_protection_officer",
             "description": "Alert DPO for potential data breach reporting",
             "auto": True},
            {"step": 5, "action": "assess_data_classification",
             "description": "Determine sensitivity level of exfiltrated data",
             "auto": False, "requires_approval": True},
        ]
    },
    "agent_down_response": {
        "name":        "Agent Down Response",
        "triggers":    ["agent_down"],
        "min_severity":"CRITICAL",
        "steps": [
            {"step": 1, "action": "verify_agent_status",
             "description": "Confirm agent is genuinely down not just delayed heartbeat",
             "auto": True},
            {"step": 2, "action": "attempt_agent_restart",
             "description": "Send restart command to agent host",
             "auto": True},
            {"step": 3, "action": "enable_compensating_controls",
             "description": "Increase monitoring on neighboring agents",
             "auto": True},
            {"step": 4, "action": "notify_soc_team",
             "description": "Alert SOC about monitoring gap",
             "auto": True},
        ]
    },
}
...
class OrchestratorAgent:
    def __init__(self):
        ...
        self._consumer = KafkaConsumerClient(
            CONSUMER_GROUP, [Topics.HQ_CORRELATED, Topics.SOAR_RESPONSES], BOOTSTRAP)
    ...
    def _build_app(self) -> FastAPI:
        ...
        async def execute_playbook(request: Request):
            ...
            if not playbook_id or playbook_id not in PLAYBOOKS:
                return JSONResponse({"error": "Unknown playbook",
                                     "available": list(PLAYBOOKS.keys())})
            pb = PLAYBOOKS[playbook_id]
            manual_incident = {
                "correlation_id":   "MANUAL-" + uuid.uuid4().hex[:8].upper(),
                "correlation_type": "manual_trigger",
                "severity":         pb["min_severity"],
                "description":      "Manual execution triggered from dashboard",
                "timestamp":        datetime.now(timezone.utc).isoformat(),
            }
            commands = []
            for step in pb["steps"]:
                ...
                cmd = {
                    ...
                    "action":            step["action"],
                    "description":       step["description"],
                    "target":            step.get("target_field", "campus"),
                    "playbook_name":     pb["name"],
                    "issued_by":         AGENT_ID,
                    "requires_approval": step.get("requires_approval", False),
                }
            ...
            return JSONResponse({
                ...
                "correlation_id":  manual_incident["correlation_id"],
            })
