...
CREDENTIAL_PATHS = {
    ...
    "library/keychains", "login.keychain", "master.passwd",
    "keychain-db", "/private/etc/master.passwd",
...
class EdrAgent:
    ...
    def handle_event(self, topic: str, payload: dict):
        ...
        if event_type == "file_op":
            ...
            self._check_credential_dump(host_id, payload, ts)
            self._check_persistence(host_id, payload, ts)
            self._check_fim(host_id, payload, ts)
    ...
    def _check_credential_dump(self, host_id, payload, ts):
        ...
        proc_hit = any(c in proc for c in ("lsass", "procdump", "mimikatz", "wce", "security", "dump-keychain"))
    ...
    def start(self):
        threading.Thread(target=self._heartbeat_loop,
                         daemon=True, name="edr-heartbeat").start()
