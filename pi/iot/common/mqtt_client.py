...
class SecureMQTTClient:
    def __init__(
        ...
        import os
        _has_certs = (
            ca_cert and client_cert and client_key
            and os.path.isfile(str(ca_cert))
            and os.path.isfile(str(client_cert))
            and os.path.isfile(str(client_key))
        )
        if _has_certs:
            self.client.tls_set(
                ca_certs=ca_cert,
    ...
    def _on_message(self, client, userdata, msg):
        raw = msg.payload
        if not raw:
            logger.debug(f"⏭️  Empty payload on {msg.topic} — skipped")
            return
        try:
            ...
            logger.error(f"❌ Message parse error on topic '{msg.topic}' payload={raw[:80]!r}: {e}")
