...
class PACEdaAgent:
    ...
    def handle_event(self, topic: str, payload: dict):
        ...
        hour      = payload.get("hour", datetime.now(TZ_LOCAL).hour)
        did       = payload.get("device_id", "unknown")
        ts        = time.time()

        logger.info(f"📡 [{access.upper():7s}] card={card_uid} floor={floor} reason={reason}")

        # ── Tamper / vibration alert ──────────────────────────────────────────
        if access == "tamper_alert":
            alerts = [self._make_alert(
                payload, "physical_tamper_detected", SeverityLevel.CRITICAL, 0.98,
                {"door_id": payload.get("door_id", did), "floor": floor,
                 "mitre": MITRE_ACCESS},
                ["lock_door", "alert_security", "notify_pac_manager",
                 "notify_hq_immediately"],
            )]
            self._stats["tamper"] += 1
            for alert in alerts:
                ...
                logger.warning(f"🚨 [CRITICAL] physical_tamper_detected door={did}")
            return  # skip all other checks for tamper events
        ...
        # ── Insider threat: card owner ≠ face → block card immediately ───────
        if reason == "insider_threat_detected":
            block_cmd = {
                "action":     "block_card",
                "card_uid":   card_uid,
                "reason":     "insider_threat_detected",
                "user_name":  payload.get("user_name", ""),
                "door_id":    payload.get("door_id", did),
                "timestamp":  datetime.now(TZ_LOCAL).isoformat(),
            }
            self._producer.publish(Topics.SOAR_COMMANDS, block_cmd, key=card_uid)
            self._producer.flush()
            alert = self._make_alert(
                payload, "insider_threat_card_blocked", SeverityLevel.CRITICAL, 0.97,
                {"card_uid":    card_uid,
                 "user_name":   payload.get("user_name", ""),
                 "door_id":     payload.get("door_id", did),
                 "action_taken": "card_blocked",
                 "mitre":       MITRE_ACCESS},
                ["block_card", "alert_security", "notify_pac_manager",
                 "notify_hq_immediately"],
            )
            self._producer.publish(Topics.PAC_ALERTS, alert, key=card_uid)
            self._recent_alerts.append(alert)
            ...
            self._stats["alerts"] += 1
            logger.warning(
                f"🚨 [CRITICAL] insider_threat_card_blocked — card {card_uid} blocked")
            return
    ...
    def _heartbeat_loop(self):
        ...
        while True:
            try:
                ...
                logger.warning(f"💓 Heartbeat sent (uptime {int(time.time()-start)}s, #{self._stats['heartbeats_sent']})")
    ...
    def start(self):
        ...
        threading.Thread(target=self._heartbeat_loop,
                         daemon=True, name="pac-eda-heartbeat").start()
        logger.info(f"▶️  PAC-EDA Agent — health :{HEALTH_PORT}/health")
        uvicorn.run(self._app, host="0.0.0.0", port=HEALTH_PORT, log_level="warning", log_config=None)
